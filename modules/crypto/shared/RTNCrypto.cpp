...
static std::string bytesToBase64(const std::vector<uint8_t>& bytes) {
  size_t encoded_len = sodium_base64_encoded_len(bytes.size(), sodium_base64_VARIANT_ORIGINAL);
  std::vector<char> encoded(encoded_len);
  sodium_bin2base64(encoded.data(), encoded_len, bytes.data(), bytes.size(), sodium_base64_VARIANT_ORIGINAL);
...
static std::vector<uint8_t> base64ToBytes(const std::string& encoded_string) {
  ...
  if (sodium_base642bin(bytes.data(), bytes.size(),
                        encoded_string.c_str(), encoded_string.size(),
...
jsi::String RTNCrypto::cryptoBoxEasy(jsi::Runtime& rt,
                                     jsi::String message,
                                     jsi::String nonce,
  ...
  validateBufferLength(rt, pkBytes, crypto_box_PUBLICKEYBYTES, "Recipient public key");
  validateBufferLength(rt, skBytes, crypto_box_SECRETKEYBYTES, "Sender secret key");
...
jsi::String RTNCrypto::cryptoBoxOpenEasy(jsi::Runtime& rt,
  ...
  validateBufferLength(rt, nonceBytes, crypto_box_NONCEBYTES, "Nonce");
  validateBufferLength(rt, pkBytes, crypto_box_PUBLICKEYBYTES, "Sender public key");
  validateBufferLength(rt, skBytes, crypto_box_SECRETKEYBYTES, "Recipient secret key");
  ...
  if (crypto_box_open_easy(decrypted.data(),
                           ...
                           pkBytes.data(),
                           skBytes.data()) != 0) {
    throw jsi::JSError(rt, "crypto_box_open_easy failed (verification failed or corrupted ciphertext)");
  }
  
  return jsi::String::createFromUtf8(rt, std::string(decrypted.begin(), decrypted.end()));
}
...
// Constants
double RTNCrypto::getCryptoBoxPublickeybytes(jsi::Runtime& rt) {
  return static_cast<double>(crypto_box_PUBLICKEYBYTES);
}

double RTNCrypto::getCryptoBoxSecretkeybytes(jsi::Runtime& rt) {
  return static_cast<double>(crypto_box_SECRETKEYBYTES);
}

double RTNCrypto::getCryptoBoxNoncebytes(jsi::Runtime& rt) {
  return static_cast<double>(crypto_box_NONCEBYTES);
}

double RTNCrypto::getCryptoBoxMacbytes(jsi::Runtime& rt) {
  return static_cast<double>(crypto_box_MACBYTES);
}
