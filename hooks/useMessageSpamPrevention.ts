...
export const useMessageSpamPrevention = (
  config: Partial<SpamPreventionConfig> = {},
) => {
  ...
  const canShowMessagePreview = useCallback(
    (senderId: string): boolean => {
      ...
      const recentMessages = senderHistory.filter(
        (ts) => ts.timestamp > cutoff,
      );
  ...
  const getSenderTimeout = useCallback(
    (senderId: string): number => {
      const senderHistory = messageHistoryRef.current.get(senderId) || [];
      if (senderHistory.length === 0) return 0;

      const oldestMessage = Math.min(
        ...senderHistory.map((ts) => ts.timestamp),
      );
      const now = Date.now();
